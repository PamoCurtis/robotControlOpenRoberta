...
public class cPropFromORB
{
	...
	private final Motor[]  motor  = new Motor[4];
	private final Sensor[]  sensor = new Sensor[4];
	private float     Vcc  = 0;
	...
	public boolean get( ByteBuffer data )
	{
		...
		synchronized( this )
			...
			for (int i = 0; i < 4; i++) {
                                motor[i].pwr   = (short)( (data.get(idx++) & (byte)0xFF) );
				motor[i].speed = (short) (((short) data.get(idx++) & 0xFF)
						| ((short) data.get(idx++) & 0xFF) << 8);
				motor[i].pos = ((int) data.get(idx++) & 0xFF)
			...
			for (int i = 0; i < 4; i++) {
				...
				sensor[i].value = ((int) data.get(idx++) & 0xFF)
						                 | ((int) data.get(idx++) & 0xFF) <<  8
						                 | ((int) data.get(idx++) & 0xFF) << 16
				                         | ((int) data.get(idx++) & 0xFF) << 24;
                ...
                sensor[i].analog1 = ((int)a & 0xFF)    	| ((int)b & 0x0F) << 8;
                sensor[i].analog2 = ((int)c & 0xFF)<<4 	| ((int)b & 0xF0) >> 4;
			...
			for (int i = 0; i < 4; i++) {
                sensor[i].isValid = (isValid & (1 << (i))) != 0;
                sensor[i].pin5    = (digital & (1 << (2 * i))) != 0;
                sensor[i].pin6    = (digital & (1 << (2 * i + 1))) != 0;
